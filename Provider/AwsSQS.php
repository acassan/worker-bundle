...
class AwsSQS extends BaseProvider
{
    ...
    public function createQueue($queueName, array $queueOptions = [])
    {
        ...
        $response = $this->parseResponse($this->sqs->create_queue($queueName, ['Attribute' => $attributes]));
    ...
    {
        $response = $this->parseResponse($this->sqs->get_queue_attributes($this->getQueueUrl($queueName), ['AttributeName' => 'All']));
    ...
    {
        try {
            $this->parseResponse($this->sqs->get_queue_url($queueName));
    ...
    public function get($queueName, $timeout = null)
    {
        ...
        do {
            $queueUrl = $this->getQueueUrl($queueName);
            $workload = $this->parseResponse($this->sqs->receive_message($queueUrl))->ReceiveMessageResult->Message;
            if ($workload) {
